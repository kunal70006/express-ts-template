...
import { ProductOrCollectionDetails } from "../controllers";
...
const getRecommendedProducts = async ({
    ...
    url: string;
    productId: string;
}): Promise<Omit<ProductOrCollectionDetails, "productRecommendations">[]> => {
    try {
...
export const getProductDetails = async (
    url: string,
): Promise<ProductOrCollectionDetails> => {
    try {
        const res = await axios.get(`${url}.json`);
        const data = await res.data;
        const productId = data.product?.id;
        const returnObj: ProductOrCollectionDetails = {
            description: "",
            font: "",
            image: "",
        ...
        if (productId) {
            ...
            returnObj.description = htmlToPlainText(data.product.body_html);
...
export const getCollectionDetails = async (url: string): Promise<void> => {
    try {
        const res = await axios.get(`${url}.json`);
        const data = await res.data;
        console.log(data);
